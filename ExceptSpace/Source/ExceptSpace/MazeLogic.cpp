...
{
	// Rozpoczynamy od lewego górnego rogu (0,0)
	DFS(0, 0);
}
...
void MazeLogic::DFS(int32 x, int32 y)
{
	...
	// Przygotowujemy losowo uporz¹dkowan¹ listê kierunków (0: góra, 1: dó³, 2: lewo, 3: prawo)
	...
	for (int32 Dir : Directions)
	{
		...
		// Sprawdzamy, czy (nx, ny) jest w granicach labiryntu i czy nie by³a odwiedzona
		if (nx >= 0 && nx < Width && ny >= 0 && ny < Height && !Maze[ny][nx].bVisited)
		{
			// Usuwamy œciany miêdzy bie¿¹c¹ komórk¹ a now¹ komórk¹
			if (Dir == 0)
			{
